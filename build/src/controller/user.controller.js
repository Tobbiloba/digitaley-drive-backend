...
exports.registerUserFunc = exports.registerUserController = exports.updateEmailSentStatusController = exports.deleteUserController = exports.getAllUserController = exports.updatePasswordController = exports.updateUserInfoController = exports.updateAccessToken = exports.logoutUserController = exports.loginUserController = void 0;
...
exports.loginUserController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
        ...
        user.authentication.sessionToken = (0, auth_1.authentication)(newSalt, user._id?.toString());
...
exports.logoutUserController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
...
exports.updateAccessToken = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
...
exports.updateUserInfoController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
...
exports.updatePasswordController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
        const { oldPassword, newPassword } = req.body;
        if (!oldPassword || !newPassword) {
            return next(new ErrorHandler_1.default('Please enter both old and new passwords', 400));
        }
        const user = await user_model_1.default
            .findById(req.user?._id)
            .select('+authentication.password');
        if (!user || !user.authentication.password) {
            return next(new ErrorHandler_1.default('User not found or invalid user', 400));
        }
        const { salt, password: hashedPassword } = user.authentication;
        ...
        const newSalt = (0, auth_1.random)();
        user.authentication.password = (0, auth_1.authentication)(newSalt, newPassword);
...
exports.getAllUserController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
...
exports.deleteUserController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
        const { id } = req.params;
...
exports.updateEmailSentStatusController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
...
exports.registerUserController = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
        ...
        if ((role === 'teacher' || role === 'user') && !courseName) {
            return next(new ErrorHandler_1.default('Please provide a course name', 400));
        }
        if (role === 'user' && !amountPaid) {
            return next(new ErrorHandler_1.default('Please provide the amount paid', 400));
        }
        ...
        if (role === 'teacher' && course && course.teacherId) {
            return next(new ErrorHandler_1.default('Course already has a teacher', 400));
        }
        const courseId = course ? course._id : null;
        if (user && role === 'user') {
            if (course) {
                if (user &&
                    user.courses.some((course) => new mongodb_1.ObjectId(course.courseId).equals(courseId))) {
        ...
        if (!user) {
            user = await user_model_1.default.create({
                email,
                courses: role === 'admin' || role === 'super admin' ? [] : [{ courseId }],
                role,
                authentication: {
                    salt,
        ...
        if (course?.contentId) {
            ...
            const content = await (0, content_model_1.getContentById)(contentId.toString());
        ...
        const paymentData = {
            courseId: courseId ? new mongoose_1.default.Types.ObjectId(courseId) : undefined,
            userId: user._id,
            payment_info: {
...
const registerUserFunc = async ({ firstName, lastName, phoneNumber, gender, email, balance, courseId, }) => {
    try {
