...
exports.getCoursesNameController = exports.searchCourse = exports.getAllCourses = exports.getCourseDetails = exports.getUserCourse = exports.getSubscribedCourse = exports.deleteCourseById = exports.updateCourse = exports.initializeCourse = void 0;
...
exports.initializeCourse = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
        const { name, description, thumbnail, courseInfo, visibility, availability, } = req.body;
        ...
        savedCourse.contentId = new mongoose_1.default.Types.ObjectId(content._id);
        content.courseIdentifier = new mongoose_1.default.Types.ObjectId(courseId);
        await content.save();
        for (const categoryName of categoryNames) {
            const category = await category_model_1.default.findOne({
                name: categoryName.trim(),
            ...
            if (category &&
                !category.courses.includes(new mongoose_1.default.Types.ObjectId(courseId))) {
                category.courses.push(new mongoose_1.default.Types.ObjectId(courseId));
...
const updateCourse = async (req, res) => {
    try {
        ...
        const updatedCourse = await course_model_1.default.findByIdAndUpdate(req.params.courseId, req.body, { new: true });
        if (!updatedCourse)
...
const deleteCourseById = async (req, res) => {
    try {
        const course = await (0, course_model_1.getCourseByIdModel)(req.params.courseId);
        if (!course)
            return res.status(404).json({ message: 'Course not found' });
        const deletedCourse = await (0, course_model_1.deleteCourseModel)(req.params.courseId);
        if (!deletedCourse)
            return res.status(404).json({ message: 'Course not found' });
...
exports.getSubscribedCourse = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
        ...
        const subscribedCourses = await Promise.all(user.courses.map(async (course) => {
            console.log(course._id.toString());
            const courseDetails = await (0, course_model_1.getCourseByIdModel)(course.courseId.toString());
        ...
        const visibleCourses = subscribedCourses.filter((course) => course !== null);
...
exports.getUserCourse = (0, catchAsynchError_1.default)(async (req, res, next) => {
    try {
...
const getAllCourses = async (req, res) => {
    try {
        ...
        const query = visible === 'false' ? { visibility: false } : { visibility: true };
