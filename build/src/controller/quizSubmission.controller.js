...
exports.deleteAllQuizSubmissionsController =
  exports.deleteQuizSubmissionController =
  exports.markQuizSubmissionController =
  exports.createQuizSubmissionController =
  exports.getQuizSubmissionsByStudentController =
  exports.getQuizSubmissionByIdController =
  exports.getAllQuizSubmissionsController =
    void 0;
...
const getAllQuizSubmissionsController = async (req, res, next) => {
  try {
    const submissions = await (0,
    quizSubmission_model_1.getAllQuizSubmissionsModel)();
...
const getQuizSubmissionByIdController = async (req, res, next) => {
  try {
    ...
    quizSubmission_model_1.getQuizSubmissionByIdModel)(submissionId);
    if (!submission) {
...
const getQuizSubmissionsByStudentController = async (req, res, next) => {
  try {
    ...
    const submissions = await (0,
    quizSubmission_model_1.getQuizSubmissionsByStudentId)(
...
const createQuizSubmissionController = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next(
        new ErrorHandler_1.default('Please provide all required fields', 400),
      );
    }
    ...
    // @ts-ignore
    const newSubmission = await (0,
    quizSubmission_model_1.createQuizSubmissionModel)(submissionData);
    res.status(201).json({
      success: true,
...
const markQuizSubmissionController = async (req, res, next) => {
  try {
    ...
    if (score == null) {
      return next(
        new ErrorHandler_1.default(
          'Score is required to grade the submission',
          400,
        ),
      );
    }
    const updatedSubmission = await (0,
    quizSubmission_model_1.markQuizSubmissionModel)(submissionId, score);
...
const deleteQuizSubmissionController = async (req, res, next) => {
  try {
    const { submissionId } = req.params;
    const deletedSubmission = await (0,
    quizSubmission_model_1.deleteQuizSubmissionModel)(submissionId);
