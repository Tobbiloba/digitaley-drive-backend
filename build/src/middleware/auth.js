...
// authenticated user
exports.isAuthenticated = (0, catchAsynchError_1.default)(
  async (req, res, next) => {
    const authHeader =
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return next(
        new ErrorHandler_1.default('Please login to access this resource', 401),
      );
    }
    ...
    if (!accessToken) {
      return next(
        new ErrorHandler_1.default('Invalid token. Please login again.', 401),
      );
    }
...
exports.hasRefreshToken = (0, catchAsynchError_1.default)(
  async (req, res, next) => {
    ...
    if (!decoded) {
      return next(
        new ErrorHandler_1.default('refresh token is not valid', 400),
      );
    }
    // check if the access token is expired
    if (decoded.exp && decoded.exp <= Date.now() / 1000) {
